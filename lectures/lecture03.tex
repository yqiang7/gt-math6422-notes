    ...
    \item Let $X$ be an algebraic
      ...
      \begin{itemize}
        ...
        \item $\ell_Z =$
          ...
          $\ell_Z(U) = \{\varphi \in \OO_X(U) : \varphi|_Z = 0\}$.
      \end{itemize}
...
\begin{definition}
  ...
  \[
    \begin{tikzcd}
      \mathcal{F}(U) \ar[d, swap, "\mathrm{res}"] \ar[r, "{f(U)}"]
      & \mathcal{G}(U) \ar[d, "\mathrm{res}"] \\
      \mathcal{F}(V) \ar[r, swap, "{f(V)}"]
      & \mathcal{G}(V)
    \end{tikzcd}
  \]
\end{definition}
...
\begin{prop}
  If $\mathcal{F}$ is a presheaf
  on $X$, then there exists a morphism
