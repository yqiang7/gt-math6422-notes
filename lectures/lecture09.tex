...
\begin{prop}\label{prop:generate}
  The following are equivalent:
  \begin{enumerate}
    \item $s_0, \dots, s_n \in \Gamma(X, \mathcal{L})$
      generate $\mathcal{L}$;
    \item $(s_0)_x, \dots, (s_n)_x$ generate $\mathcal{L}_x$ as an $\OO_{X, x}$-module
      for all $x \in X$;
    \item the morphism
...
\begin{proof}
  ...
  $(s_0)_x, \dots, (s_n)_x$ generate $\mathcal{L}_x$ as an $\OO_{X, x}$-module (2).
  This happens if and only if
  $\varphi_x$ is surjective, which happens
  if and only if $\varphi$ is surjective (3).
  This happens if and only if
  $\varphi(U)$ is surjective for all
  $U \subseteq X$ open affine (4)
