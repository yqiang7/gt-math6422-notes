...
\begin{example}
  Let $\mathcal{G}$ be a sheaf. Define the
...
\begin{remark}
  ...
  \]
  with $f_{I, J}$
  smooth complex-valued functions. Then
    $d : \mathcal{A}^k_\C \to \mathcal{A}^{k + 1}_\C$
  decomposes  as $d = \partial + \overline{\partial}$, where
  \[
  ...
  \]
  and $\overline{\partial}$ is defined similarly.
  ...
  \begin{enumerate}
    ...
    \item $\partial \mathcal{A}^{p, q} \subseteq \mathcal{A}^{p + 1, q}$ and $\overline{\partial} \mathcal{A}^{p, q} \subseteq \mathcal{A}^{p, q + 1}$;
